...
def handle_data(context):
    ...
    if str(now_datetime)[11:19] > '09:00:00' and str(now_datetime)[11:19] < '15:30:00':
        ...
        # 计算MACD，得到三个时间序列数组，diff, dea和macd_hist
        ...
        if macd_hist.iloc[-1] > 0 and macd_hist.iloc[-2] <= 0:
            order_id = context.open_long(symbol_exchange=context.universe, price=price, volume=5)
        # 柱线由正变负时，作为卖信号
        elif macd_hist.iloc[-1] < 0 and macd_hist.iloc[-2] > 0:
            order_id = context.close_long(symbol_exchange=context.universe, price=price, volume=5)
