...
def handle_data(context):
    ...
    if df_close[-2] < UP and df_close[-1] > UP:
        context.open_short_market(symbol_exchange=context.universe, volume=5)

    # 如果收盘价上穿布林上轨时，平仓
    elif df_close[-2] > DN and df_close[-1] < DN:
        context.close_short_market(symbol_exchange=context.universe, volume=5)
